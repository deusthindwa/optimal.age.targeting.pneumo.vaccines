...
fit_model <- function(x){
  ...
  start = list(beta  = coef(model0)[2])

  #fit and NLS model
  nls(data = x,
  incidence ~ exp(beta*agey),
...
A <- 
  ...
  geom_point(data = ipd, aes(x = agey, y = obs), size = 1, color = "white") +
  #geom_linerange(data = ipd, aes(x = agey, ymin = obs_lci, ymax = obs_uci)) +
  geom_line(aes(x = agey, y = `50%`), size = 1) +
  geom_ribbon(aes(x = agey, y = `50%`, ymin = `2.5%`, ymax = `97.5%`), alpha = 0.2, color = NA) +
  facet_grid(country  ~ serogroup, scales = "free_y") +
  scale_y_continuous(limits = c(0, NA), labels = ~sprintf("%g", .)) +
  labs(x = "Age (years)", y = "IPD incidence per 100,000 population") +
...
# incidence plot
ggsave(here("output", "S8_Fig_ipd_incidence.png"),
       plot = A,
